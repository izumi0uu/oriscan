...
export default function Home() {
  ...
  const { data, error, isLoading } = useSWR<HomepageResponse>('/api/homepage', fetcher)

  if (error) return 'Something went wrong ʕ•̠͡•ʔ Please try reloading the page'
  if (!hasMounted || isLoading || !data) return <Loading className="min-h-screen" />
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.2 }}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 auto-rows-fr gap-2">
            ...
            >
              ...
              <p>
                Estimated to start <span className="whitespace-nowrap">March 18, 2024</span>
              </p>
            </motion.div>
          </div>
          <div className="mt-10 mx-auto max-w-lg space-y-4">
            <p>
              Bitcoin is partitioned into periods in which the block reward is adjusted. These periods are referred to
              as &quot;Halving Periods&quot;. Every 210,000 blocks the block reward is halved.
            </p>
            <p>
              Currently, the reward per block is 6.25 BTC. This means 62,500,000,000 satoshis are added to the total
              supply of Bitcoin, for every block that is mined to the canonical chain.
            </p>
          </div>
        </motion.div>
      </main>
