...
import { Activity } from '@/components/brc20-activity-list'
import Copy from '../copy'

const Brc20ListMobile: FC<{ data: Activity[] }> = ({ data }) => {
  return (
    <div>
      {data.map((item, index) => {
        return <Brc20ListItemMobile key={index} data={item} />
      })}
    </div>
  )
}
export default Brc20ListMobile

const Brc20ListItemMobile: FC<{ data: Activity }> = ({ data }) => {
  return (
    <div className="mt-[8px]">
      ...
      <div className="text-[#3498DB] text-[12px] mt-[3px] flex">
        ...
        <span className="flex gap-x-[5px]">
          <img src="https://cdn.name3.net/name3/coins/btc.svg" alt="" width={12} /> {data.volume}
        </span>
      </div>
      ...
      <div className="text-[black] text-[12px] mt-[5px] flex">
        <span className="text-[#9F9F9F] w-[70px] block">Token:</span>
        <span>{data?.tick}</span>
      </div>
      <div className="text-[black] text-[12px] mt-[5px] flex">
        <span className="text-[#9F9F9F] w-[70px] block">Amount:</span>
