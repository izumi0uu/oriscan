...
function Blocks({ blocks }: { blocks: [string, number][] }) {
  ...
  const selectSat = useCallback(
    function (this: SVGRectElement) {
      ...
      const block = this.getAttribute('data-block')?.split('<br/>')[0].replace('#', '')
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              The following inscriptions are on sats coinbased in block #{selectedBlock}
            </DialogDescription>
          </DialogHeader>
...
const BlocksSVG = React.memo(
  function BlocksSVG({ blocks, selectSat }: { blocks: [string, number][]; selectSat: (this: SVGRectElement) => void }) {
    ...
    useLayoutEffect(() => {
      ...
      for (const [kStr, v] of blocks) {
        ...
        let rect = svg
          ...
          .attr('data-block', `#${kStr}<br/>${v} inscribed sat${v > 1 ? 's' : ''}`)
