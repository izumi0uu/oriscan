...
import { blockHeightToMinedSats, blockHeightToRewardSats } from '../utils/helpers'

import './Sats.css'

const size = 17

function SatsGrid({ block, width, sats }: { block: number; width: number; sats: string[] }) {
  ...
  useLayoutEffect(() => {
    ...
    const svg = d3.select(svgRef.current).attr('width', '100%').attr('height', '100%').call(tip)
  ...
  return (
    <>
      <div className="tools">
        <button onClick={() => firstRef.current?.scrollIntoView()}>Jump to Sat &rarr;</button>
      </div>
      <div className="pattern" style={{ width: Number(w) * size, height: Number(h) }}>
        <svg ref={svgRef} />
      </div>
    </>
  )
