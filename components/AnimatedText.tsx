...
const AnimatedText = ({ children }: AnimatedTextProps) => {
  const originalText = useRef(([] as string[]).concat(children).join('').split(''))
  ...
  useEffect(() => {
    ...
    const remaining = Math.min(NB_OF_EXTRA_RANDOM_CHARS, originalText.current.length - currentLength)
    if (text.length <= originalText.current?.length + remaining && text.join('') !== originalText.current.join('')) {
      ...
      timeout.current = setTimeout(
        ...
        */
          const newValue = [
            ...originalText.current.slice(0, Math.max(0, currentLength + progress - remaining)),
