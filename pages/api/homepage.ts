...
export default function handler(req: NextApiRequest, res: NextApiResponse<HomepageResponse>) {
  // count inscribed blocks per period
  const inscriptionsPerPeriod = Object.entries(dataPeriods).map(([key, value]) => {
    return [key, Object.entries(value).reduce((total, [_, v]) => total + v, 0)]
  })
